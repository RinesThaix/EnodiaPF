...
class MovementProcessor internal constructor(
    ...
    private fun initialize(
        ...
        if (retries == hub.maxRetries) {
            if (importance.teleportOnFail) {
                val pos = destination() ?: return false
                hub.entityTeleport(entity, pos)
            }
            return true
        }
        ...
        ReusablePoint.fromPoint(entity.position).use { currentPosition ->
            ...
            if (continueFollowing != null && lastTarget != null && !continueFollowing(entity, lastTarget, distanceSquared)) {
                clearLastData(true)
                if (importance.teleportOnFail) {
                    hub.entityTeleport(entity, nextDestination)
                }
                return true
            }
        }
        ...
        when (result.status) {
            PathfindingResult.Status.FAILED -> {
                if (result.path.isEmpty()) {
                    if (importance.teleportOnFail) {
                        destination()?.let { hub.entityTeleport(entity, it) }
                    }
                    stop()
                } else {
            ...
            PathfindingResult.Status.PARTIAL -> {
                if (result.path.isEmpty() && importance.teleportOnFail) {
                    destination()?.let { hub.entityTeleport(entity, it) }
                    stop()
                } else {
                    initialize(path.last(), destination, range, importance, retries + 1, false)
